...
// MTR2026 table fields and queries
export const mtrSelectFields = '"#", PCODE, VISIDATE, VISITIME, PNAME, PBIRTH, AGE, PHONENUM, SEX, SERIAL, N, GUBUN, RESERVED, FIN, TEMPERATUR';
...
export const getMtrDeleteSQL = (tableName: string = 'MTR2026'): string => `DELETE FROM ${tableName} WHERE "#" = ?`;
export const getMtrDeleteByPcodeVisidateSQL = (tableName: string = 'MTR2026'): string => `DELETE FROM ${tableName} WHERE PCODE = ? AND VISIDATE = ?`;
export const getMtrByIdSQL = (tableName: string = 'MTR2026'): string => `${getMtrSelectSQL(tableName)} WHERE "#" = ?`;
...
export const getWaitSelectSQL = (tableName: string = 'WAIT2026'): string => `SELECT ${waitSelectFields} FROM ${tableName}`;
export const getWaitByVisidateSQL = (tableName: string = 'WAIT2026'): string => `${getWaitSelectSQL(tableName)} WHERE VISIDATE = ?`;
export const getWaitInsertSQL = (tableName: string = 'WAIT2026'): string => `INSERT INTO ${tableName} (PCODE, VISIDATE, RESID1, RESID2, ROOMCODE, ROOMNM, DEPTCODE, DEPTNM, DOCTRCODE, DOCTRNM, GOODOC) VALUES (?, ?, ?, ?, '1', ?, '14', ?, '63221', ?, '')`;
export const getWaitUpdateSQL = (tableName: string = 'WAIT2026'): string => `UPDATE ${tableName} SET RESID1=?, RESID2=? WHERE PCODE=? AND VISIDATE=?`;
export const getWaitDeleteSQL = (tableName: string = 'WAIT2026'): string => `DELETE FROM ${tableName} WHERE PCODE = ? AND VISIDATE = ?`;